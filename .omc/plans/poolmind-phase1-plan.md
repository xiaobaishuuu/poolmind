...
## 执行方式约束（用户要求，2026-07-18；2026-07-21 细化颗粒度，约束所有执行阶段）

1. **逐功能审核门（2026-07-21 细化为前端/后端/其他各一档）**：不再以"§13 条目/功能块"为最粗颗粒度一次性做完才停——**前端算一个单元、后端算一个单元、其他（数据库/部署/测试/文档等）也各算一个单元**，每完成一个单元即停下，向用户演示可核验的结果（截图/可点的页面/测试输出），获用户审核通过后才继续下一个单元。用户是产品主管（非程序员），演示用业务语言。完整的团队分工、单元切法示例、标准流程见 `docs/DEV-PROCESS.md`（正式开发阶段的团队协作事实来源）。
